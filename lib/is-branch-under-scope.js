...
// }
function isBranchUnderScope(needleBranch, haystackBranch, options) {
	...
	debug(`wereConditionalsMet=${wereConditionalsMet} but ignoring?=${opts.ignoreConditionals}`);

	let isCurrentDescendantPieceMatched = false;
	...
	const areGroupsMatching = needleGroups.every(function(needleGroup) {
		...
		return availableHaystackGroups.some(function(haystackGroup, haystackGroupIndex) {
			...
			if(isMatching) {
				const lastHaystackNode = haystackGroup[haystackGroup.length - 1];
				const isLastHaystackNodeDescendant = lastHaystackNode.type !== 'combinator' || (
					lastHaystackNode.type === 'combinator' && (
						lastHaystackNode.value === ' ' ||
						lastHaystackNode.value === '>>' ||
						lastHaystackNode.value === '>'
					)
				);

				// Variables can flow to descendants but not siblings
				// We duplicate `isMatching` here just to make the logic a bit easier to follow
				isCurrentDescendantPieceMatched = isMatching && isLastHaystackNodeDescendant;

				currentHaystackGroupIndex = currentHaystackGroupIndex + haystackGroupIndex + 1;
			}

			debug(`group isMatching=${isMatching} haystackGroup=${currentHaystackGroupIndex}/${haystackGroups.length} isCurrentDescendantPieceMatched=${isCurrentDescendantPieceMatched}`);

			return isMatching;
		});
	});

	return (wereConditionalsMet || opts.ignoreConditionals) && areGroupsMatching && isCurrentDescendantPieceMatched;
}
